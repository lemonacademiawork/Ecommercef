...
export function OAuthSuccessPage({ onLogin }) {
  ...
  useEffect(() => {
    ...
    // Set token temporarily so profile call can use it
    localStorage.setItem("token", token);
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#FFFDF7" }}>
      ...
      >
        <div className="flex justify-center">
          {status === "loading" ? (
            <div className="relative flex items-center justify-center w-16 h-16 rounded-full bg-primary/10 text-primary" style={{ backgroundColor: "rgba(123, 31, 162, 0.1)", color: "#7b1fa2" }}>
              <Loader2 className="w-8 h-8 animate-spin" />
            </div>
          ) : (
            <div className="relative flex items-center justify-center w-16 h-16 rounded-full bg-destructive/10 text-destructive animate-bounce" style={{ backgroundColor: "rgba(220, 38, 38, 0.1)", color: "#dc2626" }}>
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-bold tracking-tight" style={{ fontFamily: "Poppins, sans-serif" }}>
            {status === "loading" ? "Completing Sign In" : "Authentication Failed"}
          </h2>
...
export function OAuthFailurePage() {
  const navigate = useNavigate();
  const location = useLocation();
  const [errorMsg, setErrorMsg] = useState("An error occurred during authentication.");

  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const err = params.get("error");
    if (err) {
      setErrorMsg(decodeURIComponent(err).replace(/_/g, " "));
    }
  }, [location]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#FFFDF7" }}>
      ...
      >
        <div className="flex justify-center">
          <div className="relative flex items-center justify-center w-16 h-16 rounded-full bg-destructive/10 text-destructive" style={{ backgroundColor: "rgba(220, 38, 38, 0.1)", color: "#dc2626" }}>
            <AlertCircle className="w-8 h-8" />
          </div>
        </div>
