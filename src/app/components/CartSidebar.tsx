...
export function CartSidebar({ isOpen, onClose, items, onUpdateQuantity, onRemove, navigate }: CartSidebarProps) {
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between p-5 border-b border-border">
              <div>
                <h2 className="font-bold text-lg" style={{ fontFamily: 'Poppins, sans-serif' }}>My Cart</h2>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center py-10">
                  ...
                  <p className="text-sm text-muted-foreground mb-5">Add some amazing craft supplies!</p>
                  <button
                ...
                items.map(item => (
                  ...
                  >
                    <img src={item.image} alt={item.name} className="w-16 h-16 rounded-xl object-cover flex-shrink-0 bg-muted" />
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center border border-border rounded-lg bg-white overflow-hidden">
                          <button onClick={() => onUpdateQuantity(item.id, item.quantity - 1)} className="w-7 h-7 flex items-center justify-center hover:bg-muted transition-colors">
                        ...
                        <button onClick={() => onRemove(item.id)} className="text-muted-foreground hover:text-destructive transition-colors p-1">
              ...
              <div className="border-t border-border p-5 space-y-3">
                ...
                <div>
                  <div className="flex gap-2">
                    ...
                    <button onClick={handleCoupon} className="px-3 py-2 rounded-xl text-sm font-semibold bg-muted hover:bg-primary hover:text-white transition-all">
                      Apply
                    </button>
                  </div>
                  {couponError && <p className="text-xs text-destructive mt-1">{couponError}</p>}
                  {appliedCoupon && <p className="text-xs text-accent mt-1">✓ LEMON20 applied — 20% off!</p>}
                </div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {discount > 0 && (
                    <div className="flex justify-between text-accent">
                      <span>Discount (LEMON20)</span><span>-₹{discount}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-muted-foreground">
