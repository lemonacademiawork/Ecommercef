...
import { ShoppingCart, Heart, Search, Menu, X, User, Package, Settings, LogOut, ChevronDown } from "lucide-react";
...
export function Navbar({
  currentPage, navigate, cartItems, wishlistCount,
  isLoggedIn, isAdmin, onCartOpen, onLogout, searchQuery, onSearchChange,
}: NavbarProps) {
    ...
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-border shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          >
            <div className="w-9 h-9 rounded-xl flex items-center justify-center text-xl shadow-sm" style={{ background: 'linear-gradient(135deg, #D81B8A, #e91ea0)' }}>
              🍋
            </div>
            <div className="hidden sm:block">
              <span className="text-xl font-bold" style={{ fontFamily: 'Poppins, sans-serif', color: '#D81B8A' }}>Lemon</span>
              <span className="text-xl font-bold" style={{ fontFamily: 'Poppins, sans-serif', color: '#2E7D32' }}>House</span>
            </div>
          </button>
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            {isAdmin && (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  currentPage === 'admin' ? 'bg-accent/10 text-accent' : 'text-foreground/70 hover:text-accent hover:bg-accent/10'
                }`}
              >
                Admin
              </button>
          ...
          <div className="hidden lg:flex items-center relative">
            ...
            <input
              ...
              value={searchQuery}
              onChange={e => { onSearchChange(e.target.value); if (e.target.value) navigate('shop'); }}
                ...
                <AnimatePresence>
                  {userMenuOpen && (
                    ...
                    >
                      ...
                      <div className="py-1">
                        <button onClick={() => { navigate('dashboard'); setUserMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-muted transition-colors">
                          <Package className="w-4 h-4 text-primary" /> My Orders
                        </button>
                        <button onClick={() => { navigate('dashboard'); setUserMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-muted transition-colors">
                          <Settings className="w-4 h-4 text-primary" /> Settings
                        </button>
                        <button onClick={() => { onLogout(); setUserMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-destructive hover:bg-red-50 transition-colors">
              ...
              {navLinks.map(link => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                    currentPage === link.page ? 'bg-primary/10 text-primary' : 'hover:bg-muted'
                  }`}
                >
                  {link.label}
                </button>
