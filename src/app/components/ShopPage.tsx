...
export function ShopPage({ navigate, onAddToCart, wishlist, onToggleWishlist, searchQuery }: ShopPageProps) {
  ...
  const FilterPanel = () => (
    <div className="space-y-6">
        ...
        <div className="space-y-1">
          <button
            ...
            className={`w-full text-left px-3 py-2 rounded-xl text-sm transition-all ${
              selectedCategory === 'all' ? 'bg-primary/10 text-primary font-semibold' : 'hover:bg-muted text-foreground/70'
            }`}
          >
            All Products
          ...
          {CATEGORIES.map(cat => (
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded-xl text-sm transition-all ${
                selectedCategory === cat.id ? 'bg-primary/10 text-primary font-semibold' : 'hover:bg-muted text-foreground/70'
              }`}
            >
              {cat.icon} {cat.name}
              <span className="float-right text-xs text-muted-foreground">{cat.count}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {activeFiltersCount > 0 && (
        <button
          onClick={() => { setSelectedCategory('all'); setPriceRange([0, 2000]); setOnlyInStock(false); }}
    ...
    <div className="min-h-screen" style={{ background: '#FFFDF7' }}>
      ...
      <div className="bg-white border-b border-border py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-bold mb-1" style={{ fontFamily: 'Poppins, sans-serif' }}>Shop All Products</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          ...
          >
            ...
            Filters
            {activeFiltersCount > 0 && (
              <span className="w-5 h-5 bg-primary text-white text-xs rounded-full flex items-center justify-center">{activeFiltersCount}</span>
            )}
          </button>
        </div>
          ...
          <aside className="hidden lg:block w-56 flex-shrink-0">
            <div className="bg-white rounded-2xl border border-border p-5 sticky top-24">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-bold" style={{ fontFamily: 'Poppins, sans-serif' }}>Filters</h2>
                {activeFiltersCount > 0 && (
                  <span className="w-5 h-5 bg-primary text-white text-xs rounded-full flex items-center justify-center">{activeFiltersCount}</span>
                )}
              </div>
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between mb-5">
                ...
                <button onClick={() => setFiltersOpen(false)} className="p-1.5 rounded-lg hover:bg-muted">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <FilterPanel />
